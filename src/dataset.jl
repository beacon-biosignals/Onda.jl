...
# TODO: simplify `Dataset` load/save/construct for new generic path world:
#
# Onda.load(path) --> load existing `Dataset` at `path` (old style: `Dataset(path; create=false)`)
# Onda.save(dataset::Dataset) --> basically the same as `save_recordings_file`
# Onda.Dataset(path) --> create new `Dataset` object targeting `path`, but unlike
#                        `Dataset(path; create=true)`, don't automatically write
#                        content to `path`
...
# TODO: Restore this functionality on top of the API in `paths.jl`.
#
# #####
# ##### `load`
# #####
